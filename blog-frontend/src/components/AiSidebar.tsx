...
import {
  ...
  Sparkles, Loader2, Upload, X, Maximize2, Minimize2, Check, Wand2
} from 'lucide-react';
import { chatWithGemini, generateImage, improveContent, generateSpeech, analyzeImage } from '../services/geminiService';
...
export const AiSidebar: React.FC<AiSidebarProps> = ({ onInsert, isOpen, onClose }) => {
  ...
  // Audio State - Web Speech API for real-time transcription
  ...
  const improveAndInsert = async () => {
    if (!transcript.trim()) return;

    setLoading(true);
    try {
      const improved = await improveContent(transcript, 'Clean up grammar and formatting for a blog post');
      onInsert(improved);
      setTranscript('');
    } catch (e) {
      console.error(e);
      alert('Failed to improve text. Inserting original...');
      insertTranscript();
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className={`${isExpanded ? 'w-[600px]' : 'w-80'} h-[calc(100vh-100px)] sticky top-24 bg-white dark:bg-[#0a0a0a] border border-neutral-200 dark:border-white/10 rounded-xl overflow-hidden flex flex-col shadow-2xl ml-4 transition-all duration-300 z-30`}>
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
        ...
        {/* Audio Tab - Web Speech API */}
        {activeTab === 'audio' && (
          <div className="space-y-6">
            <div className="p-4 bg-neutral-100 dark:bg-white/5 rounded-xl border border-neutral-200 dark:border-white/5">
              <div className="mb-4 text-neutral-700 dark:text-neutral-300 text-sm font-bold">Voice Transcription</div>

              {!isListening ? (
                <button
                  onClick={startListening}
                  className="w-16 h-16 rounded-full bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white border-2 border-red-500/50 flex items-center justify-center transition-all mx-auto"
                >
                  <Mic size={24} />
                </button>
              ) : (
                <button
                  ...
                  className="w-16 h-16 rounded-full bg-red-500 text-white animate-pulse flex items-center justify-center transition-all mx-auto border-2 border-red-600"
                >
            ...
            {(transcript || interimTranscript) && (
              <div className="space-y-4">
                <div className="p-4 bg-neutral-50 dark:bg-black border border-neutral-200 dark:border-white/10 rounded-lg min-h-[100px] max-h-[200px] overflow-y-auto">
                  <p className="text-sm text-neutral-700 dark:text-neutral-300 whitespace-pre-wrap">
                    {transcript}
                    <span className="text-neutral-400 dark:text-neutral-600">{interimTranscript}</span>
                  </p>
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 py-2 bg-neutral-100 dark:bg-white/5 border border-neutral-200 dark:border-white/10 text-neutral-900 dark:text-white font-bold rounded-lg text-sm hover:bg-neutral-200 dark:hover:bg-white/10 transition-colors flex justify-center items-center gap-2 disabled:opacity-50"
                  >
                    <Check size={16} /> Insert
                  </button>
                  <button
                    onClick={improveAndInsert}
                    disabled={loading || !transcript.trim()}
                    className="flex-1 py-2 bg-orange-500 text-white dark:text-black font-bold rounded-lg text-sm hover:bg-orange-600 transition-colors flex justify-center items-center gap-2 disabled:opacity-50"
                  >
                    {loading ? <Loader2 className="animate-spin" size={16} /> : <Wand2 size={16} />} AI Polish
                  </button>
                </div>

                <button
                  onClick={() => setTranscript('')}
                  className="w-full py-1.5 text-xs text-neutral-500 hover:text-red-500 transition-colors"
                >
                  Clear
                </button>
              </div>
