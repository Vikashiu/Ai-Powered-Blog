...
const RichEditor = forwardRef<RichEditorHandle, RichEditorProps>(({ content, onChange, placeholder, onAiRequest }, ref) => {
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Generate unique ID for the loading placeholder
        const loadingId = `img-loading-${Date.now()}`;

        // Insert loading placeholder
        insertHtml(`
            <div id="${loadingId}" class="my-6 p-8 rounded-lg bg-neutral-100 dark:bg-neutral-900 border border-dashed border-neutral-300 dark:border-neutral-800 flex flex-col items-center justify-center text-neutral-500 animate-pulse select-none" contenteditable="false">
                <span class="text-2xl mb-2">📤</span>
                <span class="text-sm font-medium">Uploading to Cloudinary...</span>
            </div>
        `);
        ...
        try {
            ...
            const placeholder = editorRef.current?.querySelector(`#${loadingId}`);
            if (placeholder) {
                const imgDiv = document.createElement('div');
                imgDiv.className = 'my-6 select-none group relative';
                imgDiv.contentEditable = 'false';
                imgDiv.innerHTML = `<img src="${response.url}" class="w-full rounded-xl shadow-lg" alt="Uploaded image" />`;
            ...
            if (placeholder) {
                placeholder.innerHTML = `
                    <span class="text-red-500 flex items-center gap-2 text-sm">
    ...
    const handleGalleryUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (validFiles.length === 0) {
            alert('No valid images selected. Please select images less than 5MB.');
            return;
        }

        // Generate unique ID for the loading placeholder
        const loadingId = `gallery-loading-${Date.now()}`;

        // Insert loading placeholder
        insertHtml(`
            <div id="${loadingId}" class="my-6 p-8 rounded-lg bg-neutral-100 dark:bg-neutral-900 border border-dashed border-neutral-300 dark:border-neutral-800 flex flex-col items-center justify-center text-neutral-500 animate-pulse select-none" contenteditable="false">
                <span class="text-2xl mb-2">📤</span>
                <span class="text-sm font-medium">Uploading ${validFiles.length} image(s) to Cloudinary...</span>
            </div>
        `);
        ...
        try {
            ...
            const imagesHtml = imageUrls.map(url =>
                ...
                <img src="${url}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="Gallery image" />
            </div>`
            ).join('');
        ...
        `;
            ...
            // Replace loading placeholder with gallery
            const placeholder = editorRef.current?.querySelector(`#${loadingId}`);
            if (placeholder) {
                const galleryDiv = document.createElement('div');
                galleryDiv.innerHTML = galleryHtml;
                placeholder.replaceWith(galleryDiv.firstElementChild!);
                onChange(editorRef.current?.innerHTML || '');
            }
        } catch (error) {
            ...
            if (placeholder) {
                placeholder.innerHTML = `
                    <span class="text-red-500 flex items-center gap-2 text-sm">
                        ⚠️ Gallery upload failed. Please try again.
                    </span>
                `;
                placeholder.className = "my-6 p-4 rounded-lg bg-red-500/5 border border-red-500/20 flex items-center justify-center";
            }
